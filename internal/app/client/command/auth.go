package command

import (
	"bufio"
	"context"
	"fmt"
	"io"

	"github.com/bbquite/go-pass-keeper/internal/models"
	"github.com/bbquite/go-pass-keeper/internal/service/client"
)

type AuthCommand struct {
	authService *client.ClientAuthService
	reader      io.Reader
	writer      io.Writer
}

func NewAuthCommand(authService *client.ClientAuthService, reader io.Reader, writer io.Writer) *AuthCommand {
	return &AuthCommand{
		authService: authService,
		reader:      reader,
		writer:      writer,
	}
}

func (c *AuthCommand) Name() string {
	return "AUTH"
}

func (c *AuthCommand) Desc() string {
	return "To access other commands, you need to log in with your login and password."
}

func (c *AuthCommand) Usage() string {
	return "\t\tlogin - login for account\n\t\tPassword - password for acc\n"
}

func (c *AuthCommand) Execute() error {

	var data models.UserLoginData
	scanner := bufio.NewScanner(c.reader)

	_, err := fmt.Fprint(c.writer, "Enter login: ")
	if err != nil {
		return err
	}

	if scanner.Scan() {
		data.Username = scanner.Text()
	} else {
		return scanner.Err()
	}

	_, err = fmt.Fprint(c.writer, "Enter password: ")
	if err != nil {
		return err
	}

	if scanner.Scan() {
		data.Password = scanner.Text()
	} else {
		return scanner.Err()
	}

	err = c.authService.AuthUser(context.Background(), &data)
	if err != nil {
		return err
	}

	return nil
}
